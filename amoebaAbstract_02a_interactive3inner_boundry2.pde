...
/* ...........................file paths................................................. */
String image_input_dir = "data_used_treeimages";
...
//String input_file_path = "F:/2016_PROCESSING_CODE/amoebaAbstract_02a_interactive3inner_boundry2/"+image_input_dir+"/";  //FILE SAVING DIRECTORY
...
/* show_random_chosen_images */
// boolean show_random_chosen_images = false;     /* if(show_random_chosen_images) {} */
boolean show_random_chosen_images = true;
...
float rd_min = 2;  /* The size of the images is affected by the size od the screen image */
float rd_max =4;
...
float trace_alpha = 10; 
float trace_red = 200;
float trace_green =20 ;
float trace_blue   = 1;
color  trace_color;
...
//float screen_alpha = random(5,25);
float screen_alpha ;
...
/* ............................................................. Setup lerpColor() ..................................................... */
	...
	backgroundImage = loadImage(texture_dir_path+"texture_feather25-_30opacity.png");
	...
	fileList  = getFilePathsInDirectory1(input_file_path,"png");
//	imageFileNames = new StringList();  
	for ( int i = 0; i< numImageFilesDirectory; i++ )
	{
		/*  Load files into Image ListArray */
		//String input_file_name = input_file_path+ i + ".png";
		String input_file_name = fileList .get(i);
		PImageList.add(loadImage( input_file_name) );
	...
	/* Print image names in directory */
	/* 	for (int q =0;q<imageFileNames.size();q++)
	{println ("Image ["+q+"] = "+ imageFileNames.get(q));} */
	
	/* Returns a random non repeating list to avoid duplicates. */
	listSequenceNumber = returnNonRepeatingInterger(numImageFilesDirectory, numImageFilesDirectory) ;
	/* printIntegerList(listSequenceNumber, "line 250"); */
	/*exit(); */

	if(show_random_chosen_images) {	
...
void draw() {
	...
	if(save_number_examples == save_number){		
		...
		exit();}
	/* Change color and alpha   using lerpColor*/		
	if( cycle_counter %50 == 0){
		...
		//  image(backgroundImage,0,0,width,height);
		/* Ajust fill rectangle according with scale_factor */
	...
	/* /* Change full screen alpha images */      /*changeFillAlpha  changes fill color also */ 
	...
	if(save_big_graphic) {
		...
		if(particles[0].cnt%10==0) {
			...
			rect(0,0,width*scale_factor,height*scale_factor);   /* Full screen fill */
			//rect(width/2,height/2,width,height);//700,400); // this changes opacity and fill
		}
